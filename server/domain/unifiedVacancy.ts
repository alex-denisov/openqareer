...
export interface VacancyMatchExplanation {
  ...
  /**
   * Совпадение с конкретным фактом профиля, а не просто со строкой (B248,
   * «совпадает по фактам профиля»). Тот же порядок и текст, что и у
   * `matchingPoints`; `factId` — только когда совпавший навык связан с
   * подтверждённым фактом кандидата, иначе поле у элемента не пишется.
   */
  matchingFacts?: readonly { readonly text: string; readonly factId?: string }[];
  /**
