...
function sendRepositoryError(
  ...
  if (error instanceof LinkedinPoolConflictError) {
    ...
    return sendError(reply, request, status, code, 'Операция над аккаунтом LinkedIn не выполнена.', false);
  }
  return sendError(reply, request, 500, 'linkedin_pool_operation_failed', 'Операция над аккаунтом LinkedIn не выполнена.', true);
}
...
async function handleCreate(deps: LinkedinPoolDeps, request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!key) return sendError(reply, request, 422, 'idempotency_key_required', 'Повторяемый запрос требует Idempotency-Key.', false);
  ...
  try {
    ...
    return reply.code(result.created ? 201 : 200).send({ data: result.account, meta: { requestId: request.id } });
  } catch (error) {
...
async function handlePatch(deps: LinkedinPoolDeps, request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    return { data: repository.update({ ...params, ...body, actorUserId: principal.userId, actorUsername: principal.username }), meta: { requestId: request.id } };
  } catch (error) {
    return sendRepositoryError(error, request, reply);
  }
}
...
async function handleComplete(deps: LinkedinPoolDeps, request: FastifyRequest, reply: FastifyReply) {
  const principal = requireAdminMutation(deps, request, reply);
  if (!principal) return;
  const repository = repositoryOrError(deps, request, reply);
  if (!repository) return;
  ...
  try {
    return { data: await repository.completeLogin(accountId, body.handle), meta: { requestId: request.id } };
  } catch (error) {
    return sendRepositoryError(error, request, reply);
  }
}
...
async function handleProbe(deps: LinkedinPoolDeps, request: FastifyRequest, reply: FastifyReply) {
  ...
  const { accountId } = adminLinkedinPoolParamsSchema.parse(request.params);
  try {
    return { data: await repository.probeAccount(accountId, { actorUserId: principal.userId, actorUsername: principal.username }), meta: { requestId: request.id } };
  } catch (error) {
    return sendRepositoryError(error, request, reply);
  }
}
...
async function handleRevoke(deps: LinkedinPoolDeps, request: FastifyRequest, reply: FastifyReply) {
  ...
  const { accountId } = adminLinkedinPoolParamsSchema.parse(request.params);
  try {
    return { data: repository.revoke(accountId, { actorUserId: principal.userId, actorUsername: principal.username }), meta: { requestId: request.id } };
  } catch (error) {
    return sendRepositoryError(error, request, reply);
  }
}
...
export function registerLinkedinPoolRoutes(
  app: FastifyInstance,
  deps: LinkedinPoolDeps,
): void {
  ...
  app.post('/api/v1/admin/linkedin/accounts/:accountId/session/complete', withDeps(deps, handleComplete));
