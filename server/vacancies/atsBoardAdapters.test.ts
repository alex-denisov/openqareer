...
describe('ats board source id', () => {
  ...
  it('строит адрес каждой доски по замеренному образцу', () => {
    ...
    expect(atsBoardEndpoint('breezy', 'joom-group')).toBe('https://joom-group.breezy.hr/json');
  ...
  it('разрешает SmartRecruiters, Breezy HR и Pinpoint в контрактах', () => {
    ...
    expect(
      ATS_PROVIDERS.filter((p) => p.crawlPermission === 'allowed').map((p) => p.provider),
    ).toEqual([
      'greenhouse',
      'lever',
      'ashby',
