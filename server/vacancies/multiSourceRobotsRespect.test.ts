...
describe('движок уважает robots.txt площадки', () => {
  it('запрет словами останавливает опрос, и площадка об этом сказана', async () => {
    ...
    const engine = new MultiSourceVacancyEngine({
      sources: [
        source('ats-smartrecruiters-x', 'https://api.smartrecruiters.com/v1/companies/X/postings'),
      ],
      fetcher: async () => {
        fetched += 1;
        return [];
      ...
      sources: [
        {
          ...
          robotsOverride: {
            ...
            basis:
              'API задокументирован площадкой для распространения вакансий со ссылкой на источник',
          },
        },
