...
export function CareerIntake({
  ...
  function moveFromSource() {
    if (sourceChoice === 'profile-import' && !ingested && !connectedSource) {
      setError(
        isDesktop
          ? 'Подключите профиль с площадки или выберите другой источник: «PDF», «Текстом» или «Без документов».'
          : 'Подключение площадок — в приложении для компьютера. Здесь: «PDF», «Текстом» или «Без документов».',
      );
      return;
    }
    if (
